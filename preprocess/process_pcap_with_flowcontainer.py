...
# 这个方法的作用是从原始的 pcap 文件中提取序列长度和前 n 个包的前 m 字节，并返回负载数据和序列长度数据。
def get_pay_seq(pcap, threshold, ip_length, n, m):
    """
        提取序列长度，与前n个包的前m字节,当流序列长度小于threshold忽略该流

        Args:
        - pcap: 字符串，表示原始 pcap 文件路径
        - threshold: 整数，表示序列长度的阈值，小于该阈值的序列将被忽略
        - ip_length: 整数，指定 IP 地址的长度
        - n: 整数，指定前 n 个包
        - m: 整数，指定前 m 字节

        Returns:
        - pay_load: NumPy 数组，包含提取的负载数据
        - seq_load: NumPy 数组，包含提取的序列长度数据
        """
...
"""
    ...
    Args:
    - pcap_folder: 字符串，表示 pcap 文件夹路径
    - threshold: 整数，表示序列长度的阈值，小于该阈值的序列将被忽略
    - ip_length: 整数，指定 IP 地址的长度
    - packet_num: 整数，指定每个数据包的数量
    - byte_num: 整数，指定每个数据包的字节数

    Returns:
    - pay_list: NumPy 数组，包含提取的负载数据
    - seq_list: NumPy 数组，包含提取的序列数据
    - label_list: NumPy 数组，包含提取的标签数据
"""
def getPcapIPLength(pcap_folder, threshold, ip_length, packet_num, byte_num):
    ...
    seq_list = []
    label_list = []
    for label_name in os.listdir(pcap_folder):#遍历所有类别
        label2num[label_name] = class_num
        label_path = os.path.join(pcap_folder, label_name)
        for pcap in os.listdir(label_path): # 遍历该类别内所有的pcap
            # 遍历每一个pcap,提取前packet_num个包的byte_num字节
            """
            将负载特征，序列特征，标签分别存到 pay_list, seq_list, label_list 中
            """
