...
def get_pay_seq_get_pay_seq_statis(pcap, threshold, ip_length, n, m):
    ...
    print(f"static_flow shape: {len(static_flow)}")
    print(f"pay_flow shape: {len(pay_flow)}")
    print(f"seq_flow shape: {len(seq_flow)}")
...
def getPcapMesg(pcap_folder, threshold, ip_length, packet_num, byte_num):
    ...
    # print(f"pay_flow shape: {len(pay_list)}")
    # print(f"seq_flow shape: {len(seq_list)}")
    #print(f"static_flow shape: {len(statistic_list)}")
...
def split_pcap_2_session(pcap_folder, traffic_path, splitcap_path):
    ...
    for (root, _, files) in os.walk(pcap_folder):
        ...
        for Ufile in files:
            ...
            # 检查文件是否为有效的 pcap 文件
            if pcap_suffix not in ['pcap', 'pcapng']:
                logger.warning(f"非 pcap 文件：{Ufile}")
                continue
            Ufile_list.pop()
            pcap_name = ".".join(Ufile_list)
            print(1.1)
            # try:
            #     assert pcap_suffix in ['pcap', 'pcapng'] # == 'pcap'
            # except:
            ...
            logger.info('处理完成文件 {}'.format(pcap_file_path))#Ufile))
